...
import type { IELTSTestData } from "../types/ielts";

const TESTS_PER_PAGE = 6;

const cambridgeTests = (data as IELTSTestData[]).map((item) => ({
  id: item.test_id,
  book: item.book,
  test: item.test_number,
  passages: item.passages.length,
  questions: item.passages.reduce(
    (total, p) =>
      total +
      p.question_sections.reduce((sum, s) => sum + s.questions.length, 0),
    0,
  ),
  time: 60,
}));
...
export default function Reading() {
  ...
  const totalPages = Math.ceil(cambridgeTests.length / TESTS_PER_PAGE);
  const paginatedTests = cambridgeTests.slice(
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          >
            ...
            <p className="text-text-secondary text-lg font-body pl-1">
              Official Cambridge IELTS Reading tests — 3 passages, 40 questions
              each
            </p>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="text-text-secondary font-body space-y-2 leading-relaxed">
            <p>
              ✦ Each test consists of 3 reading passages with increasing
              difficulty
            </p>
            <p>✦ You have 60 minutes to complete 40 questions</p>
            <p>
              ✦ Question types include multiple choice, true/false/not given,
              matching, and more
            </p>
            <p>
