...
import { Clock, BookOpen, Users, MessageSquare, CheckCircle2, FileText, ChevronRight } from "lucide-react";
...
export default function TestLandingClient({ testData }: Props) {
  ...
  useEffect(() => {
    const attempts = useHistoryStore.getState().getAttemptsByTestId(testData.id);
    // Sort by date descending (newest first)
    setHistory(attempts.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()));
  }, [testData.id]);

  const totalQuestions = testData.passages.reduce(
    (sum, p) => sum + p.questionGroups.reduce((gSum, g) => gSum + g.questions.length, 0),
    0
  );
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        >
          ...
          <div className="flex gap-3 mb-4">
            <span className="bg-gray-100 text-gray-600 px-3 py-1 text-xs font-bold rounded-sm border border-gray-200">#IELTS Academic</span>
            <span className="bg-gray-100 text-gray-600 px-3 py-1 text-xs font-bold rounded-sm border border-gray-200">#Reading</span>
          </div>
          ...
          <div className="flex border-b border-gray-200 mb-6">
            <button className="px-6 py-2 border-b-2 border-blue-600 text-blue-700 font-bold bg-blue-50/50">Thông tin đề thi</button>
            <button className="px-6 py-2 text-gray-500 font-medium hover:bg-gray-50 transition-colors">Đáp án/transcript</button>
          </div>

          <div className="space-y-3 text-text-secondary text-sm">
            <div className="flex items-center gap-2">
              <Clock className="w-4 h-4" /> Thời gian làm bài: 60 phút | {testData.passages.length} phần thi | {totalQuestions} câu hỏi | 3813 bình luận
            </div>
            <div className="flex items-center gap-2">
              <Users className="w-4 h-4" /> 1,570,503 người đã luyện tập đề thi này
            </div>
          </div>
          ...
          {history.length > 0 ? (
            <div className="overflow-x-auto border border-gray-200 rounded-sm">
              <table className="w-full text-left text-sm">
                ...
                <tbody>
                  {history.map((attempt) => (
                    <tr key={attempt.id} className="border-b border-gray-100 hover:bg-gray-50/50 transition-colors">
                      <td className="p-4">
        ...
        >
          ...
          <div className="flex items-center gap-2 mb-8 border-b border-gray-100 pb-4">
            ...
            <span className="bg-gray-100 text-gray-600 text-sm py-0.5 px-3 rounded-full font-bold">3,813</span>
          </div>
            ...
            <div className="flex gap-4">
              ...
              <div className="flex-1">
                <div className="bg-gray-50 p-5 rounded-lg rounded-tl-none border border-gray-200 shadow-sm">
                  ...
                  <p className="text-gray-700 leading-relaxed">Đề này Passage 3 phần True/False/Not Given khó quá, mình làm sai gần hết 😭 Mọi người có tips nào cho dạng này không ạ?</p>
                </div>
              ...
              <div className="flex-1">
                <div className="bg-gray-50 p-5 rounded-lg rounded-tl-none border border-gray-200 shadow-sm">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <span className="text-xs text-gray-400 font-medium">1 ngày trước</span>
                  </div>
                  <p className="text-gray-700 leading-relaxed">Đề sát với cấu trúc thi thật, cám ơn admin đã tổng hợp.</p>
                </div>
                <div className="flex gap-4 mt-2 ml-2 text-xs font-bold text-gray-500 uppercase tracking-wide">
                  <button className="hover:text-blue-600 transition-colors">Thích</button>
                  <button className="hover:text-blue-600 transition-colors">Phản hồi</button>
